...
class BaseSoC(SoCMini): 
	...
	my_led_pad = platform.request("user_led_n")
        ...
        self.sync+=[If(self.MMIO_inst.res_st_reg.re == True, wdt_count.eq(self.MMIO_inst.res_st_reg.fields.watchdog),my_led_pads.eq(0))]
        #if external reset is tue the watchdog is reset, if watchdog is zero, reset all peripherals
        self.sync+=[If(external_reset==True,wdt_count.eq(0),global_reset.eq(True)).Elif(wdt_count==0,global_reset.eq(True),my_led_pads.eq(1)).Else(global_reset.eq(False),If(wdt_psc==0,wdt_count.eq(wdt_count-1)))]
